...
def test_negative_offset_limit(tmp_path, mcp_server):
    ...
    tf.write_text("A\nB\nC\nD\nE")
    result = api_read_file(mcp_server["url"], str(tf), limit=0)
    assert result.strip().startswith("A")
    result2 = api_read_file(mcp_server["url"], str(tf), offset=-5)
    assert "A" in result2
    result3 = api_read_file(mcp_server["url"], str(tf), offset=500)
